		...
		if ( $subscriptions_shippings ) :
			...
			<tr>
				...
				<?php
					...
					echo wp_kses_post( wc_price( $subscriptions_shippings['cost'], array( 'currency' => $subscription->get_order_currency() ) ) . sprintf( _x( '<small> via %s</small>', 'placeholder shipping name', 'yith-woocommerce-subscription' ), $subscriptions_shippings['name'] ) );
				?>
					</td>
			</tr>
			<?php
			...
			foreach ( $subscription->get_order_ids() as $order_id ) :
				...
				<li>
					...
					if ( ! $current_order ) {
						...
						printf( wp_kses_post( _x( '<p>Order #%d</p>', 'order id', 'yith-woocommerce-subscription' ) ), esc_html( $order_id ) );
						continue;
					}
