...
<tr class="ywsbs-recurring-totals-items">
	...
	<td>
		<?php foreach ( WC()->cart->get_cart_contents() as $cart_item_key => $cart_item ):
			...
			if ( isset( $cart_item['ywsbs-subscription-info'] ) ):
				...
				$recurring_price = wc_get_price_to_display( $cart_item['data'], array( 'qty' => $cart_item['quantity'], 'price' => $recurring_price ) );
				$recurring_tax = '';
				...
				$price_html .= wc_price( $recurring_price ) .  ' / ' . $recurring_period . ' '.$recurring_tax;
