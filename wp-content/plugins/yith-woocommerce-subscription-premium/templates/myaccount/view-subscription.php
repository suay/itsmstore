...
wc_print_notices();
// var_dump($subscription->get_status());exit;
...
// var_dump($subscription->can_be_resubscribed());exit;
?>
<div class="ywsbs-subscription-view-wrap">
	...
	<div class="ywsbs-subscription-info-wrapper">
		...
		<div class="ywsbs-box ywsbs-subscription-info-box subscription-info">
			...
			<?php if ( ! in_array( $status, array( 'paused', 'cancelled' ) ) && $subscription->get_payment_due_date() ) : ?>
				<div class="ywsbs-subscription-info-item">
			...
			<div class="ywsbs-subscription-info-item status-item">
				<strong><?php esc_html_e( 'Status:', 'yith-woocommerce-subscription' ); ?></strong> <?php printf( '<span class="status %1$s">%2$s</span>', $status, esc_attr( ucfirst( $subscription_status ) ) ); ?>
		...
		<div class="ywsbs-box ywsbs-subscription-info-box subscription-totals">
			...
			<table class="subscription-review-table">
				...
				<tfoot>
				<?php
				if ( $subscription->get_line_tax() != 0 ) :
				...
				if ( ! empty( $subscriptions_shippings ) ) :
					...
					<tr>
						...
						<td><?php echo wc_price( $subscriptions_shippings['cost'], array( 'currency' => $subscription->get_order_currency() ) ) . sprintf( __( '<small> via %s</small>', 'yith-woocommerce-subscription' ), $subscriptions_shippings['name'] ); ?></td>
					</tr>
					<?php
					if ( ! empty( $subscription->get_order_shipping_tax() ) ) :
						...
						<tr>
							...
							<td><?php echo wc_price( $subscription->get_order_shipping_tax(), array( 'currency' => $subscription->get_order_currency() ) ); ?></td>
						</tr>
					<?php
					endif;
				endif;
				...
				<tr>
					...
					<td><?php echo wc_price( $subscription->get_subscription_total(), array( 'currency' => $subscription->get_order_currency() ) ); ?></td>
				</tr>
				</tfoot>
			</table>
		...
		<div class="ywsbs-box ywsbs-subscription-related-orders">
			...
			<table class="shop_table ywsbs_subscription_table my_account_orders shop_table_responsive">
				...
				foreach ( $order_ids as $order_id ) :
					...
					if ( ! $order ) :
						...
						<tr>
							<td class="order-number"
								...
								<?php echo esc_html_x( '#', 'hash before order number', 'yith-woocommerce-subscription' ) . $order_id; ?>
							</td>
							<td class="order-date"
					...
					$next_payment_due_date = ( ! in_array( $subscription->get_status(), array( 'paused', 'cancelled' ) ) && $subscription->get_payment_due_date() ) ? date_i18n( wc_date_format(), $subscription->get_payment_due_date() ) : '';
					...
					<tr>
						...
						<td class="order-date"
							data-title="<?php esc_attr_e( 'Date', 'yith-woocommerce-subscription' ); ?>">
							<time datetime="<?php echo esc_attr( date( 'Y-m-d', strtotime( $order_date ) ) ); ?>"
						...
						<td class="order-status"
							data-title="<?php esc_attr_e( 'Status', 'yith-woocommerce-subscription' ); ?>">
						<span
							class="status-<?php echo esc_attr( $order->get_status() ); ?>"><?php echo wc_get_order_status_name( $order->get_status() ); ?></span>
						</td>
	...
	<div class="ywsbs-subscription-info-wrapper">
		<div class="ywsbs-box ywsbs-subscription-info-box billing-info">
			...
			<div class="ywsbs-subscription-info-item">
				...
				<br><?php echo WC()->countries->get_formatted_address( $billing_address ); ?>
			</div>
		</div>
			...
			<div class="ywsbs-box ywsbs-subscription-info-box shipping-info">
				...
				<div class="ywsbs-subscription-info-item">
					<?php if ( $shipping_formatted ) : ?>
						<strong><?php esc_html_e( 'Address:', 'yith-woocommerce-subscription' ); ?></strong>
						<br><?php echo $shipping_formatted; ?>
					<?php else : ?>
				...
				<div class="ywsbs-box ywsbs-subscription-info-box delivery-schedules">
					...
					<table class="my_account_orders shop_table_responsive">
						...
						foreach ( $ds as $scheduled ):
							...
							?>
							<tr>
								<td data-title="<?php esc_html_e( 'Shipped on', 'yith-woocommerce-subscription' ); ?>"><?php echo ywsbs_get_formatted_date( $scheduled->scheduled_date, '-' ); ?></td>
								<td data-title="<?php esc_html_e( 'Delivery Status', 'yith-woocommerce-subscription' ); ?>"><span
										class="delivery-status <?php echo esc_attr( $scheduled->status ); ?>"><?php echo YWSBS_Subscription_Delivery_Schedules()->get_status_label( $scheduled->status ); ?></span>
								</td>
								<td data-title="<?php esc_html_e( 'Delivery on', 'yith-woocommerce-subscription' ); ?>"><?php echo ywsbs_get_formatted_date( $scheduled->sent_on, '-' ); ?></td>
							</tr>
						<?php endforeach; ?>
						</tbody>
					</table>
