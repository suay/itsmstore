...
<div class="ywsbs-subscription-delivery-schedules">
	...
	<?php else :
		$status = YWSBS_Subscription_Delivery_Schedules()->get_status();
		?>
		...
		<table class="ywsbs-delivery-schedules-table widefat striped">
			...
			<tbody>
			<?php foreach ( $delivery_schedules as $delivery ) : ?>
				<tr>
					<td>
						<?php echo esc_html( ywsbs_get_formatted_date( $delivery->scheduled_date, '-' ) ); ?>
					</td>
					<td class="table-hide">
						<?php echo YWSBS_Subscription_Delivery_Schedules()->get_status_label( $delivery->status ); ?>
					</td>
					<td class="status-td" data-id="<?php echo esc_attr( $delivery->id ); ?>">
						<div class="status-normal"
							data-value="<?php esc_attr( $delivery->status ); ?>"><?php echo YWSBS_Subscription_Delivery_Schedules()->get_status_label( $delivery->status ); ?></div>
						<select class="status-hover">
							<?php foreach ( $status as $key => $single_status ): ?>
								<option value="<?php echo esc_attr( $key ); ?>"
									data-label="<?php echo $single_status; ?>" <?php selected( $delivery->status, $key ); ?>><?php echo esc_html( $single_status ); ?></option>
							<?php endforeach; ?>
						</select>
					</td>
					<td>
