	...
	<?php else : ?>
		...
		<table class="ywsbs-history-table widefat striped">
			...
			<tr>
				...
				<th><?php esc_html_e( '', 'yith-woocommerce-subscription' ); ?></th>
			...
			<?php foreach ( $history as $current_order ) : ?>
				<tr>
					...
					<td>
						<?php echo $current_order->get_date_created()->date_i18n( apply_filters( 'woocommerce_admin_order_date_format', __( 'M j, Y', 'yith-woocommerce-subscription' ) ) ); ?>
						</td>
					<td>
					...
					<td>
						<?php echo wc_price( $current_order->get_total(), array( 'currency' => $current_order->get_currency() ) ); ?>
					</td>
				</tr>
			<?php endforeach; ?>
			</tbody>
