...
if ( ! class_exists( 'YWSBS_Subscription_My_Account' ) ) {
	...
	class YWSBS_Subscription_My_Account {
		...
		public function subscriptions_page() {
			...
			echo YWSBS_Subscription_Shortcodes::my_account_subscriptions_shortcode(); // phpcs:ignore
		}
...
// var_dump($subscription->can_be_cancelled(),$subscription->can_be_repay() );exit;
			...
			if ( $subscription->can_be_cancelled() ) {
				...
				//add new 20-8-21 set if status == suspend change button name
			...
			if ( $subscription->can_be_repay() ) {
				...
				$modal_repay_text = '<b>Are you sure you need to continue this payment?</b>';//get_option('ywsbs_text_resume_subscription_modal');//get_option( 'ywsbs_text_cancel_subscription_modal' );

				
				$repay = array(
					'dropdown_text'      => $dropdown_text,
					'modal_text'         => $modal_repay_text,
					'button_label'       => esc_html__( 'Re-Pay', 'yith-woocommerce-subscription' ),
					'modal_button_label' => 'Yes',//get_option( 'ywsbs_text_cancel_subscription_button' ),
					'close_modal_button' => 'No',//get_option( 'ywsbs_text_close_modal' ),
					'nonce'              => '',//wp_create_nonce( 'ywsbs_cancel_subscription' ),
				); 
			}


			if ( ! $cancel && ! $pause && ! $resume && ! $repay ) {
				return false;
			}
		...
		public function show_subscription_switch( $subscription ) {
			...
			if ( $subscription->can_be_switchable() ) {
				...
				//edit 25/05/2021
				$switchable_variations = YWSBS_Subscription_Switch::get_switchable_variationsnew( $subscription );
		...
		public function add_subscription_menu_item( $wc_menu ) {
			...
			$wc_menu[ YITH_WC_Subscription::$endpoint ] = esc_html__( 'Subscriptions', 'yith-woocommerce-subscription' );
