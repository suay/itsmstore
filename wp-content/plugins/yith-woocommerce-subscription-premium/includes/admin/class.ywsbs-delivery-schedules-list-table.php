...
class YWSBS_Delivery_Schedules_List_Table extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			case 'product':
				...
				if( $product ){
					return '<a href="' . admin_url( 'post.php?post=' . $product_page_id . '&action=edit' ) . '">' . $product->get_name() . ' (#' . $product->get_id() . ')</a>';
				}else{
			...
			<select class="status-hover">
				<?php foreach ( $status_list as $key => $single_status ) : ?>
					<option value="<?php echo esc_attr( $key ); ?>"
						data-label="<?php echo $single_status; ?>" <?php selected( $status, $key ); ?>><?php echo esc_html( $single_status ); ?></option>
				<?php endforeach; ?>
			</select>
		</div>
		<?php
	...
	protected function render_start_end_date_filter() {
		...
		echo '<input type="text" size="11"  value="' . $start_date . '" name="start_date" placeholder="yyyy-mm-dd" data-date-format="yy-mm-dd" class="range_datepicker from yith-plugin-fw-datepicker" autocomplete="off" id="start_date">';
		echo esc_html__( ' To: ', 'yith-woocommerce-subscription' );
		echo '<input type="text" size="11"  value="' . $end_date . '" name="end_date" placeholder="yyyy-mm-dd" data-date-format="yy-mm-dd" class="range_datepicker to yith-plugin-fw-datepicker" autocomplete="off" id="end_date">';
		echo '</div>';
	}
	/**
