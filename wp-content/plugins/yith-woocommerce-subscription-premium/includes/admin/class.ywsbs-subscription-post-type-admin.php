...
if ( ! class_exists( 'YWSBS_Subscription_Post_Type_Admin' ) ) {
	...
	class YWSBS_Subscription_Post_Type_Admin {
		...
		public function show_subscription_delivery_schedules( $post_type, $post ) {
			...
			$label = sprintf( __('Every %d %s - on %s', 'yith-woocommerce-subscription'), $delivery_schedules['delivery_gap'], ywsbs_get_time_options_sing_plur( $delivery_schedules['delivery_period'], $delivery_schedules['delivery_gap'] ), $each );
