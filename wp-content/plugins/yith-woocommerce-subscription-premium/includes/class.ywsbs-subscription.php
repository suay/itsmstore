	...
	class YWSBS_Subscription extends YWSBS_Subscription_Legacy {
		...
		*/
		...
		public function get_address_fields( $type = 'billing', $no_type = false, $prefix = '' ) {
			...
			if ( apply_filters( 'yith_subscription_get_address_by_order', false ) || empty( $value_to_check ) ) {
				...
				if ( $order instanceof WC_Order ) {
					...
					$message     = $indentation . ' Get the information from subscription #' . $this->get_id() . ' with user ' . $this->get_user_id() . '( Order customer: ' . $order->get_user_id() . ' )';
		...
		public function get_address_fields_from_order( $type = 'billing', $no_type = false, $prefix = '' ) {
			...
			if ( $order ) {
				$message = '--------  Get the information from original order';
				// yith_subscription_log( $message, 'subscription_payment' );
			...
			// yith_subscription_log( print_r( $fields, true ), 'subscription_payment' ); // phpcs:ignore
			return $fields;
		}
		...
		public function get_confirmed_valid_date() {
			...
			$valid_date = $valid_date < current_time( 'timestamp' ) ? 0 : $valid_date;
		...
		public function get_daily_amount() {
			...
			$_price                   = (float) $this->get( 'order_subtotal' );
			...
			$divider      = ( $_ywsbs_price_is_per * $days_in_seconds[ $_ywsbs_price_time_option ] ) / DAY_IN_SECONDS;
			$daily_amount = $divider ? $_price / $divider : 0;
			...
			if ( $new_status !== $old_status || ! in_array( $new_status, array_keys( ywsbs_get_status() ), true ) ) {
				...
				switch ( $new_status ) {
					...
					var_dump($new_status);exit;
						...
						//	call inquiry recurring
						global $wpdb;
						$findold_order = $wpdb->get_results( $wpdb->prepare("SELECT meta_value FROM {$wpdb->prefix}postmeta where post_id='".$this->id."' and meta_key='order_id'") );
						foreach ( $findold_order as $values )
						{
						   $order_ids = $values->meta_value;
						}
						$recurring_unique_id = $wpdb->get_results( $wpdb->prepare("SELECT data_res FROM log_recurring_res where order_id='".$order_ids."' order by datetimes desc limit 1") );
						foreach ($recurring_unique_id  as $unid)
						{
							$detail_recurring = $unid->data_res;
						}
						
						$getuniqueid = json_decode($detail_recurring);
						$recurring_unique_id = $getuniqueid->recurring_unique_id;
						

						/*  add new flow 08-06-2021  */
						//================= end call api cancel recurring ==========//
						$datapost = json_encode( array('subscription_id'=>$this->id,
													'type_inquiry'=>'C',
													//'order_id'=>$order_ids,
...
// exit;
		...
		public function register_failed_attempt( $attempts = false, $latest_attempt = false, $next_attempt_date = '' ) {
			...
			if ( ! $latest_attempt ) {
				...
				YITH_WC_Activity()->add_activity( $this->id, 'failed-payment', 'success', $this->get( 'order_id' ), sprintf( esc_html_x( 'Failed payment for order %d', '%d: Order id', 'yith-woocommerce-subscription' ), $this->get( 'order_id' ) ) );
			...
			if ( 'yes' === $suspend_subscription ) {
				if ( ! $this->has_status( 'suspended' ) ) {
					...
					yith_subscription_log( sprintf( 'Subscription suspended. Order %$1s. Subscription %$2s', $this->get( 'order_id' ), $this->id ) );
				}
			}
		...
		*/
		...
		public function can_be_editable( $key ) {
			...
			if ( $gateway && ! $this->has_status( $status ) ) {
		...
		public function can_be_cancelled() {
			...
			$status                  = array( 'pending', 'overdue', 'cancelled','suspended' );// edit 20-8-21 move status'suspended', 
		...
		/** new Re-Pay 21-8-21 
		 * Return if the subscription can be reject recurring by getway
		 *
		 * @return bool
		 * @since  1.0.0
		 */
		public function can_be_repay() {
			$can_be_repay        = false;
			$status                  = array( 'suspended' );// edit 20-8-21 move status'suspended', 
			$product                 = $this->get_product();
			$can_be_repay_option = "yes";//$product instanceof WC_Product && $product->get_meta( '_ywsbs_override_cancellation_settings' ) === 'yes' ? $product->get_meta( '_ywsbs_can_be_cancelled' ) : get_option( 'ywsbs_allow_customer_cancel_subscription' );

			$post_type_object = get_post_type_object( YITH_YWSBS_POST_TYPE );
			// if ( is_admin() && current_user_can( $post_type_object->cap->delete_post, $this->get_id() ) && ! $this->has_status( 'suspended' ) ) {
			// 	$can_be_repay = true;
			// } elseif ( ! $this->has_status( $status ) && 'yes' === $can_be_repay_option ) {
			// 	$can_be_repay = true;
			// }
			if( $this->has_status( 'suspended' ) && 'yes' === $can_be_repay_option ){
				$can_be_repay = true;
			}
			// var_dump($can_be_repay);exit;
			return apply_filters( 'ywsbs_can_be_cancelled', $can_be_repay, $this );
		}
