	...
	class YITH_WC_Subscription_Status extends WC_Email {
		...
		public function trigger( $subscription ) {
			...
			if ( version_compare( WC()->version, '3.2.0', '<' ) ) {
				...
				$this->replace['subscription-id']     = $subscription->get_number();
				$this->replace['subscription-status'] = isset( $status[ $subscription->get_status() ] ) ? $status[ $subscription->get_status() ] : $subscription->get_status();
			} else {
