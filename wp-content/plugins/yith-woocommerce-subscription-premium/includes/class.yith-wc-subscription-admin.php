...
if ( ! class_exists( 'YITH_WC_Subscription_Admin' ) ) {
	...
	class YITH_WC_Subscription_Admin {
		...
		public function export_shipping_list() {
			require_once YITH_YWSBS_DIR . 'vendor/autoload.php';

			$rows      = YWSBS_Subscription_Delivery_Schedules()->get_processing_delivery_schedules();

			$mpdf_args = apply_filters( 'ywsbs_mpdf_args', array(
		...
		public function check_empty_panel_options() {
			...
			$post = $_POST;
			if ( isset( $post['yit_panel_wc_options_nonce'], $post['ywsbs_enable_sync-all_products']) && wp_verify_nonce( $post['yit_panel_wc_options_nonce'], 'yit_panel_wc_options_' . $this->panel_page ) ) { //phpcs:ignore
				...
				if ( isset( $post['ywsbs_site_staging'] ) ) {
					...
					if ( $post['ywsbs_site_staging'] !== $old ) {
						yith_subscription_log( 'Changed site staging from ' . $old . ' to ' . $post['ywsbs_site_staging'] );
					}
				} else {
