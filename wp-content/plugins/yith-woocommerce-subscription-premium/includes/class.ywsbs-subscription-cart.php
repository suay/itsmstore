	...
	class YWSBS_Subscription_Cart {
		...
		public function before_calculate_totals( $cart ) {
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				if ( ywsbs_is_subscription_product( $cart_item['data'] ) && $cart_item['data']->needs_shipping() && (
						isset( $cart_item['ywsbs-subscription-info']['sync'] ) && 1 == $cart_item['ywsbs-subscription-info']['sync'] &&
						( 0 == $cart_item['data']->get_price() || ('' !== $cart_item['ywsbs-subscription-info']['fee'] && 0 == ( $cart_item['data']->get_price() - $cart_item['ywsbs-subscription-info']['fee'] ) ) )
					) ) {
						$product_id = $cart_item['data']->get_id() ;
					! in_array($product_id, $this->list_of_not_shippable ) && array_push( $this->list_of_not_shippable, $cart_item['data']->get_id() );
				} else {
					$add_filter = false;
				}
			}
		...
		public function maybe_not_shippable( $value, $product ) {
			if ( in_array( $product->get_id(), $this->list_of_not_shippable ) ) {
				return false;
			}
			return $value;
		}
		...
		public function change_prices_for_calculation( $price, $product ) {
			...
			if ( WC()->cart ) {
				...
				foreach ( WC()->cart->get_cart() as $cart_item_element ) {
					$product_in_cart   = $cart_item_element['product_id'];
					$variation_in_cart = $cart_item_element['variation_id'];
				...
				if ( $in_cart ) {
					if ( isset( $cart_item['ywsbs-subscription-info'] ) ) {
						$signup_fee   = $cart_item['ywsbs-subscription-info']['fee'];
		...
		public function get_formatted_subscription_total_amount( $product, $quantity, $subscription_info = false ) {
			...
			if ( $max_length && $max_length > 1 && 'yes' === get_option( 'ywsbs_subscription_total_amount', 'no' ) ) {
				...
				if ( ! wc_prices_include_tax() ) {
					$sbs_total_format .= ' <small class="tax_label">' . WC()->countries->inc_tax_or_vat() . '</small>';
				}
		 ...
		 * @since  1.2.0
		 */
		public function change_general_price_html( $product, $quantity = 1, $show_complete_price = false, $cart_item ) {
			...
			if ( is_cart() && ! $show_complete_price_on_substotal_cart ) {
				$signup_fee  = $subscription_info['fee'];
			...
			return apply_filters( 'ywsbs_change_subtotal_product_price', $price_html, $product, $quantity, $cart_item );
		...
		*/
		...
		public function cart_recurring_totals() {
			...
			wc_get_template( 'cart/ywsbs-recurring-totals.php', null, '', YITH_YWSBS_TEMPLATE_PATH . '/' );
