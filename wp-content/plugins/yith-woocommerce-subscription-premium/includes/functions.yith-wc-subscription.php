...
if ( ! function_exists( 'ywsbs_get_unused_subscription_days' ) ) {
	...
	function ywsbs_get_unused_subscription_days( $subscription ) {
		...
		$difference_days = 0;
		if ( $valid_date > $now ) {
			$difference_days = floor( ( $valid_date - $now ) / DAY_IN_SECONDS );
		}
...
if( !function_exists( 'ywsbs_get_trial_period' ) ){
	...
	function ywsbs_get_trial_period( $product ){
		...
		if( empty( $trial_per ) ||  $trial_per <= 0 ){
			return false;
		}

		if( $trial_time_option === 'weeks'){
			$trial_per = $trial_per * 7;
			$trial_time_option = 'days';
		}

		$trial_per_format =array(
			'days'=>'D',
			'months'=>'M',
			'years' =>'Y'
		);

		$period = 'P'.$trial_per.$trial_per_format[$trial_time_option];
