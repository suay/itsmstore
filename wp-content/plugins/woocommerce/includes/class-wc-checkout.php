...
class WC_Checkout {
	...
	public function get_posted_data() {
		...
		foreach ( $this->get_checkout_fields() as $fieldset_key => $fieldset ) {
			...
			foreach ( $fieldset as $key => $field ) {
				...
				// phpcs:enable WordPress.Security.NonceVerification.Missing
				/*
				 * if convert free to payment change value order_website before
				*/
				if( $key == 'order_website' ){

					//$cksamewebsite = explode("-", $value);
					// if( !empty(@$cksamewebsite[1]) && @$cksamewebsite !==''){
					// 	$value = $cksamewebsite[0];
					// }
					$cksamewebsite = substr($value,-1);
					if( $cksamewebsite == "`"){
		...
		try {
			...
			if ( empty( $posted_data['woocommerce_checkout_update_totals'] ) && 0 === wc_notice_count( 'error' ) ) {
				...
				if ( apply_filters( 'woocommerce_cart_needs_payment', $order->needs_payment(), WC()->cart ) ) {
					...
					//add curl api script
					// $this->process_createAWS( $order_id );
	...
	/**
	 * Get posted data call api for create AWS.
	 *
	 * @since  3.1.0
	 * @return array of data.
	 */
	public function process_createAWS( $order_id ) {
		global $wpdb;
		...
		foreach (WC()->cart->get_cart() as $cart_item_key => $values) {
			...
			//check none setting set default 10 days
			if( empty($values['data']->ywsbs_price_is_per) ){
				$setting_isper = "10";
			}else{
				$setting_isper = $values['data']->ywsbs_price_is_per;
			}
            $ispertype  = $setting_isper;  // value of per 1,2 
            $typetimeoption = $values['data']->ywsbs_price_time_option;  //type day,month,year
		}

		if($priceproduct=="0.00"){
			$valuetype = "DEMO";
			$isper = $ispertype.' '.$typetimeoption;
		}else{
			$valuetype = $plan;
		}
// var_dump($values['data']->sku,$values['data']->attributes['pa_agent'],$values['data']->status,$values['data']->price,$values['data']->ywsbs_price_is_per,$values['data']->ywsbs_price_time_option);
// exit;
