...
date_default_timezone_set('UTC');
	...
	if( empty($_REQUEST['old_order']) ){
		//$value = array('code'=>'Error','message'=>'Request Customer Name.');
		echo json_encode( array('mscode'=>'Error','message'=>'Request Old OrderID.') );
		exit;
	}
	if( empty($_REQUEST['subscription_order']) ){
		echo json_encode( array('mscode'=>'Error','message'=>'Request Subscription.') );
		exit;
	}
	if( empty($_REQUEST['detail_upgrade']) ){
		echo json_encode( array('mscode'=>'Error','message'=>'Request Detail Upgrade.') );
		exit;
	}
	if( empty($_REQUEST['cus_email']) ){
		echo json_encode( array('mscode'=>'Error','message'=>'Request Email.') );
		exit;
	}
	if( empty($_REQUEST['paydiff']) ){
		echo json_encode( array('mscode'=>'Error','message'=>'Request Amount Difference to Pay.') );
		exit;
	}
...
// Check connection
  ...
  // check receive for upgrade or downgrade
  if( empty($_REQUEST['rp']) ){

  	//for upgrade export pdf
			$find_oldorder = explode('O', $_REQUEST['order_id']);
      $date_payment = substr($find_oldorder[0], -8); //date pay
      $old_order = $find_oldorder[1]; //old order id 
      $changeyear = substr($date_payment,0,4);
      $changemonth = substr($date_payment,4,2);
      $changedays = substr($date_payment,6,2);
      $newchange_invoice = $changedays.$changemonth.$changeyear;
      $received_number = $newchange_invoice.$old_order.'00000';
      $text_pay = "Pay The Difference";
      $text_pro = "Product Upgrade";
      $textfile = "Receipt_Upgrade.pdf";
  	
	}else{
		$old_order = $_REQUEST['old_order'];
		$received_number = $_REQUEST['order_id'];
		$text_pay = "Price";
		$text_pro = "Product Downgrade";
		$textfile = "Receipt_downgrade.pdf";
		
	}

  $paydiff = number_format($_REQUEST['paydiff'],0,'.','');//(int)$data['paydiff'];// number_format($data['paydiff'],2);
  $ispaydiff = substr($paydiff,-2);
  $fispaydiff = substr($paydiff,0,-2);
  $thispaydiff = $fispaydiff.'.'.$ispaydiff;
  $conpaydiff = number_format($thispaydiff,2,'.',',');
  $exp_product = explode("to",$_REQUEST['detail_upgrade']);
  $exfrom_namepro = explode(":",$exp_product[0]);
  $exto_namepro = explode(":", $exp_product[1]);
// var_dump( $paydiff,$ispaydiff,$fispaydiff,$thispaydiff,$conpaydiff );
//  var_dump($exfrom_namepro[2],$exto_namepro[3]);
// exit;
	
			//name customer
  		$mycus = mysqli_query($mysqli, "SELECT * FROM wp_postmeta where meta_key ='_order_website' and post_id=".$old_order);
  		$rowsite = mysqli_fetch_array($mycus, MYSQLI_ASSOC);
  		$namecustomer = $rowsite['meta_value'];

	   	$result = mysqli_query($mysqli, "SELECT meta_key,meta_value FROM wp_postmeta where post_id=".$_REQUEST['subscription_order']." and meta_key='status' and meta_value='active'");
	   	$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
	   	//stats = active only
	   	//find period day,week,month,year
	   	$resultperiod = mysqli_query($mysqli, "SELECT meta_key,meta_value FROM wp_postmeta where post_id=".$_REQUEST['subscription_order']." and meta_key ='price_time_option' ");
   		...
   		$type_period = $rowck['meta_value'];
				...
				//set find subscription setting
			        switch($type_period){
			            case "days":
			            	$resultday = mysqli_query($mysqli, "SELECT meta_value FROM wp_postmeta where post_id=".$_REQUEST['subscription_order']." and meta_key='price_is_per'");
			            ...
			            break;
			            case "weeks":
			           		$resultweek = mysqli_query($mysqli, "SELECT meta_value FROM wp_postmeta where post_id=".$_REQUEST['subscription_order']." and meta_key='price_is_per'");
			            ...
			            break;
			            case "months":
				            $resultmonth = mysqli_query($mysqli, "SELECT meta_value FROM wp_postmeta where post_id=".$_REQUEST['subscription_order']." and meta_key='price_is_per'");
			            ...
			            break;
			            case "years":
			            	$resultyear = mysqli_query($mysqli, "SELECT meta_value FROM wp_postmeta where post_id=".$_REQUEST['subscription_order']." and meta_key='price_is_per'");
		  ...
		  $resultorder = mysqli_query($mysqli, "SELECT meta_key,meta_value FROM wp_postmeta where post_id='".$_REQUEST['subscription_order']."'");
			...
			while ($roworder = mysqli_fetch_array($resultorder, MYSQLI_ASSOC)) {
				...
				//find product name
				$resultnameproduct = mysqli_query($mysqli, "SELECT * FROM wp_woocommerce_order_items where order_id='".$_REQUEST['old_order']."'");
				...
				$resultcurrency = mysqli_query($mysqli, "SELECT meta_key,meta_value FROM wp_postmeta where post_id='".$_REQUEST['old_order']."'");
				...
				//find bill address
				$resultuser = mysqli_query($mysqli, "SELECT meta_value FROM wp_postmeta where post_id='".$_REQUEST['subscription_order']."' and meta_key='user_id'");
				...
				$pdf->setXY( 20, 20 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','TIS-620' , 'Netka System Company Limited (Head office)' ) );

				$pdf->SetFont('tahoma','','10');
				$pdf->setXY( 20, 30 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','TIS-620' , '1 Soi Ramkhamhaeng 166 Yeak 2, ' ) );
				...
				$pdf->setXY( 20, 40 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','TIS-620' , 'Khet Minburi, Bangkok 10510' ) );
				$pdf->setXY( 20, 45 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','TIS-620' , 'Tax ID : 0105548030573') );
				...
				$pdf->setXY( 125, 75 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , 'Subscription No.' ) );
				$pdf->setXY( 185, 75 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $_REQUEST['subscription_order'] ) );
				...
				$pdf->setXY( 125, 85 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $text_pay ) );
				$pdf->setXY( 165, 85 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $detail_order['order_currency'].' '.$conpaydiff ) );
		    ...
		    // Title
		    $pdf->Cell(180,20,iconv( 'UTF-8','cp874' , $text_pro.'                                                                               Qty                           '.$text_pay ),0,1,'L',true);//Product Price
			  ...
			  $pdf->setXY( 20, 120 );
				...
				$pdf->setXY( 35, 130 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $exfrom_namepro[2].' to '.$exto_namepro[2] ) );
				$pdf->setXY( 137, 130 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $detail_order['quantity'] ) );
				$pdf->setXY( 170, 130 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $detail_order['order_currency'].' '.$conpaydiff ) );
				// $pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $detail_order['order_currency'].' '.number_format($detail_order['line_subtotal'],2) ) );
				...
				if( !empty($detail_order['order_tax']) && $bill_customer['billing_country']=="TH" ){

					$pdf->setXY( 135, 160 );
					$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , 'Tax' ) );
					$pdf->setXY( 170, 160 );
					$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $currencys['_order_currency'].' '.$detail_order['order_tax'] ) );
					
					$pdf->setXY( 135, 170 );
					$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , 'Total' ) );
					$pdf->setXY( 170, 170 );
					$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $currencys['_order_currency'].' '.$conpaydiff ) );
				...
				// $pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $currencys['_order_currency'].' '.number_format($detail_order['line_total'],2) ) );

				// $pdf->setXY( 135, 170 );
				// $pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , 'Pay The Difference' ) );
				// $pdf->setXY( 170, 170 );
				// $pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , $currencys['_order_currency'].' '.$conpaydiff ) );
				...
				$pdf->setXY( 20, 272 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , '* Request for Tax Invoice (For Thai customer) Plese click this button (link) to fill information') );
				$pdf->setXY( 20, 275 );
				$pdf->MultiCell( 0 , 0 , iconv( 'UTF-8','cp874' , '* Request for 3% withholding TAX  (For Thai Customer) Plese click this button (link) to fill information') );

				// $pdf->Ln(20);
				
				// $pdf->Output();
				// exit;
				$pdfdoc = $pdf->Output('save/'.$textfile, "F");
				// call function send email 
			  $filename  = $textfile;//"Receipt.pdf";
				$path      = "save/";
				$file      = $path . $filename;
...
// exit;
				...
				$replyto   = "";
				$mailbcc   = "";
				$mailto    = "wattawadee.raksachon@gmail.com"; //$_request['cus_email'];
